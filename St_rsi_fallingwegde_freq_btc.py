...
# ---------------------------------------------
# Define last local minima functions for RSI
# --------------------------------------------
...
# ---------------------------------------------
# Define max functions for price 
# ---------------------------------------------

def find_local_max(df_resampled, candlel_pos, neighbor=5):
    
    # find local max
    local_max_ref = df_resampled.iloc[candlel_pos]['price']['max']
    local_max = local_max_ref
    for pos in range(candlel_pos-neighbor, candlel_pos+neighbor):
        if df_resampled.iloc[pos]['price']['max'] > local_max_ref:
            local_max = df_resampled.iloc[pos]['price']['max']

    if local_max == local_max_ref:
        local_max_pos = candlel_pos
        return local_max_pos, local_max_ref
    else:
        return -1,-1
    

def find_max_in_lookback(df_resampled,rolling_candle, lookback, neighbor):
    
    max_pos = []
    max_prices = []
    for pos in range(rolling_candle-lookback + neighbor, rolling_candle-neighbor):
        local_max_pos, local_max = find_local_max(df_resampled, pos, neighbor=5)
        if local_max_pos>0:
            max_pos.append(local_max_pos)
            max_prices.append(local_max)

    return max_pos, max_prices

def calculate_max_trendline(max_pos, max_prices):
    if len(max_prices) > 1:
        slmax, intercmax = np.polyfit(max_pos,max_prices,1)
        trendmax = np.polyval([slmax, intercmax], max_pos)
        return slmax,intercmax,trendmax
    else:
        return []
    
    
def get_max_line_price(slmax, intercmax,rolling_candle,max_pos,max_prices,trendmax):
    trendmax_current = np.polyval([slmax, intercmax], rolling_candle)
    max_pos.append(rolling_candle)
    max_prices.append(trendmax_current)
    trendmax = np.append(trendmax,trendmax_current)
    return trendmax_current,max_pos,max_prices,trendmax


def dectect_breakout_max(df_resampled, rolling_candle, trendmax_current):
    if ((df_resampled.iloc[rolling_candle]['price']['last'] > trendmax_current) and 
    (df_resampled.iloc[rolling_candle]['price']['first'] < trendmax_current)):
       return rolling_candle
    else:
        return -1
...
# Storage metrics in a dictionary
...
# ----------------------------
# Backtest with reinvesting
# ----------------------------

# -------- Backtest with reinvesting --------

# Define the investment percentage per trade
investment_pct_per_trade = 0.2  # Investment is 20% of current capital
...
# Initialize the columns for the capital
df_range['capital_5'] = initial_capital
df_range['capital_10'] = initial_capital

# Calculate the new capital after each trade
for i in range(1, len(df_range)):
    df_range['capital_5'].iloc[i] = df_range['capital_5'].iloc[i-1] * (1 + df_range['strategy_return_with_fee_5'].iloc[i])
    df_range['capital_10'].iloc[i] = df_range['capital_10'].iloc[i-1] * (1 + df_range['strategy_return_with_fee_10'].iloc[i])

# Calculate the hold out return
df_range['cum_hold_out_return_5'] = (df_range['return_5'] + 1).cumprod() * initial_capital 
df_range['cum_hold_out_return_10'] = (df_range['return_10'] + 1).cumprod() * initial_capital 
...
with open('../Backtest/backtest_metrics_btc.pkl', 'wb') as f:
